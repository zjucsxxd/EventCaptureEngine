/****************************************************************************
** Meta object code from reading C++ file 'filedownloader.h'
**
** Created: Thu Sep 26 16:48:02 2013
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.4)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../src/filedownloader.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'filedownloader.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.4. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_FileDownloader[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      16,   15,   15,   15, 0x08,
      36,   15,   15,   15, 0x08,
      63,   15,   15,   15, 0x08,
      96,   15,   15,   15, 0x08,
     114,   15,   15,   15, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_FileDownloader[] = {
    "FileDownloader\0\0btnDownload_click()\0"
    "downloadListWidget_click()\0"
    "downloadListWidget_doubleclick()\0"
    "btnDelete_click()\0btnClose_click()\0"
};

const QMetaObject FileDownloader::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_FileDownloader,
      qt_meta_data_FileDownloader, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &FileDownloader::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *FileDownloader::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *FileDownloader::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_FileDownloader))
        return static_cast<void*>(const_cast< FileDownloader*>(this));
    return QDialog::qt_metacast(_clname);
}

int FileDownloader::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: btnDownload_click(); break;
        case 1: downloadListWidget_click(); break;
        case 2: downloadListWidget_doubleclick(); break;
        case 3: btnDelete_click(); break;
        case 4: btnClose_click(); break;
        default: ;
        }
        _id -= 5;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
